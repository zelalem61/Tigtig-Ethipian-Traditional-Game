...
from startScreen import air_start, disp_text
...
# Create game objects.
paddle1 = Player(const.PADDLE1X, const.PADDLE1Y,  pygame.image.load("assets/player1.png"))
paddle2 = Player(const.PADDLE2X, const.PADDLE2Y, pygame.image.load("assets/player2.png"))
# puck = Puck(width / 2, height / 2)
puck = Ball(width / 8, height / 2)
...
def init():
    ...
    pygame.display.set_caption('ምትትር')
    ...
    roundfont = pygame.font.Font('AbyssinicaSIL-Regular.ttf', 45)
...
def score(score1, score2, player_1_name, player_2_name):
    ...
    text1 = smallfont.render("{0} : {1}".format('አቤ', str(score1)), True, const.BLACK)
    text2 = smallfont.render("{0} : {1}".format('ከቤ', str(score2)), True, const.BLACK)


    screen.blit(text1, [40, 0])
    screen.blit(text2, [width - 230, 0])


def rounds(rounds_p1, rounds_p2, round_no):
    disp_text(screen, "ዙር "+str(round_no), (width/2, 20), roundfont, const.BLACK)
...
def notify_round_change():
    while True:
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if event.key == K_SPACE:
                    return
        round_text = roundfont.render("ያለቀ ዙር {0}".format(round_no), True, colors[2][0])
        ...
        cont_text = smallfont.render("ቀጥል", True, const.WHITE)
...
def show_pause_screen():
    ...
    while True:
        text_pause = smallfont.render("አቁም", True, const.BLACK)
        ...
        if (mouse[0] > width / 4) and (mouse[0] < width / 4 + 150) and (mouse[1] > height - 200) and \
                (mouse[1] < height - 160):
            pygame.draw.rect(screen, colors[4][0], (width / 4, height - 200, 220, 50))
            if click[0] == 1:
                return 2
        else:
            pygame.draw.rect(screen, colors[4][1], (width / 4, height - 200, 220, 50))
        text_restart = smallfont.render("አዲስ ጨዋታ", True, const.WHITE)
        screen.blit(text_restart, [width / 4 + 30, height - 195])

        # CONTINUE
        if (mouse[0] > width / 2 - 70) and (mouse[0] < width / 2 + 80) and (mouse[1] > height - 200) and \
                (mouse[1] < height - 160):
            pygame.draw.rect(screen, colors[0][0], (width / 2 - 70, height - 200, 150, 50))
            if click[0] == 1:
                return 1
        else:
            pygame.draw.rect(screen, colors[0][1], (width / 2 - 40, height - 200, 150, 50))
        text_cont = smallfont.render("ቀጥል", True, const.WHITE)
        screen.blit(text_cont, [width / 2 - 20, height - 195])

        # EXIT
        if (mouse[0] > width / 2 + 150) and (mouse[0] < width / 2 + 300) and (mouse[1] > height - 200) and \
                (mouse[1] < height - 160):
            pygame.draw.rect(screen, colors[1][0], (width / 2 + 150, height - 200, 150, 50))
            if click[0] == 1:
                pygame.quit()
                sys.exit()
        else:
            pygame.draw.rect(screen, colors[1][1], (width / 2 + 150, height - 200, 150, 50))
        text_exit = smallfont.render("ውጣ", True, const.WHITE)
...
def render_field(background_color):
    ...
    if background_color == (255, 169, 119):
        # Render Logic
        ...
        # goals
        pygame.draw.rect(screen, const.BLACK, (5, const.GOAL_Y1, 5, const.GOAL_WIDTH))
        pygame.draw.rect(screen, const.BLACK, (width - 10, const.GOAL_Y1, 5, const.GOAL_WIDTH))
        # Divider
...
if __name__ == "__main__":
    ...
    init()
    while True:
        gameChoice, player1_color, player2_color, mute, player_1_name, player_2_name = air_start(
